/* 1991.2.1   modified for Nemacs x68k by icam. */
/* 1991.10.17 modified for Nemacs on GNUDOS by boo@soum.co.jp */
     ...
     char **argv;
{
...
#if defined(HUMAN) || defined(GNUDOS)
  i = 1;
  ...
  else if (argc > i + 1 && !strcmp (argv[i], "-a"))
    {
...
#endif /* not HUMAN and not GNUDOS */
...
#if defined(HUMAN) || defined(GNUDOS)
    return scan_lisp_file (filename, 1);
#else
    return scan_lisp_file (filename);
#endif /* not HUMAN and not HUMAN */
  else if (!strcmp (filename + len - 3, ".el"))
#if defined(HUMAN) || defined(GNUDOS)
    return scan_lisp_file (filename, 0);
#else
    return scan_lisp_file (filename);
#endif /* not HUMAN and not HUMAN */
  else
...
#if defined(HUMAN) || defined(GNUDOS)
scan_lisp_file (filename, binmode)
     char *filename;
     int binmode;
#else
...
#if defined(HUMAN) || defined(GNUDOS)
  infile = fopen (filename, binmode ? "rb" : "rt");
#else
  infile = fopen (filename, "r");
#endif /* not HUMAN and not GNUDOS */
  if (infile == NULL)
